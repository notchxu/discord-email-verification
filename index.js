...
client.on('message', message => {
  ...
  if (message.channel.id === CONFIG.VERIFY_CHANNEL_ID && message.content === '!verify') {
    message.author
      .createDM()
      .then(dmchannel =>
        dmchannel.send('To gain access to the MIT Course 18 Discord, please enter your full MIT email address.').catch(reason => console.log(reason))
      )
      .catch(reason => console.log(reason))
  } else if (message.type === 'GUILD_MEMBER_JOIN') {
    message.channel
      .send("<@" + message.author.id + ">, welcome to the MIT Course 18 Discord! Please type '!verify' in #verification to verify your email.")
      .catch(reason => console.log(reason))
  }
...
sendEmail = (email_address, code) =>
  Email.send({
    ...
    Body: 'Welcome to the MIT Course 18 Discord! Your code is: ' + code
  })
